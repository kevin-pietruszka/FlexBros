...
import { getUserExercises, getExerciseHistory, checkPaidUser } from "../db";
...
const ExerciseStats: React.FC = () => {
    ...
    useEffect(() => {
        ...
        checkPaidUser(userID).then((result) => {
            setPaid(result);
        });
    }, [userID]);
    ...
    useEffect(() => {
        ...
        getExerciseHistory(userID, selectedExercise)
            .then((res) => {
                processHistory(res?.exercise.history, res?.start, res?.days);
            })
    ...
    if (paid) {
        return (
            <IonPage>
                ...
                <IonContent id="exercise_graph">
                    ...
                    <IonItem>
                        <IonSelect
                            onIonChange={progressSelect}
                            interface="popover"
                            placeholder="Select exercise"
                        >
                            <IonSelectOption value="reps">Reps</IonSelectOption>
        ...
        return (
            <IonPage>
                ...
                <IonContent>
                    <IonText>
                        Please upgrade to use stats viewer
                    </IonText>
                </IonContent>
            </IonPage>
        );
